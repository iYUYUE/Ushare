...
function sndFilterReq(action, table) {
    ...
    $( ".SBSSIDfilter" ).click(function() {
      // alert($(this)[0].innerHTML);
      document.getElementsByName("BSSID1").innerHTML = $(this)[0].innerHTML;
      filterTable(this, 'Ushare_Track', 'BSSID1', '');
    });
...
function getFormValues(fobj,valFunc) {
	...
	for(var i = 0;i < fobj.elements.length;i++) {
		...
		if (element_type == 'text' || element_type == 'textarea'){
			...
			if(fobj.elements[i].name=="BSSID1"||fobj.elements[i].name=="BSSID2")
				// alert(fobj.elements[i].name);
				str += fobj.elements[i].name + "=" + cleanseStrForURIEncode(myAddSlashes(mac2int(fobj.elements[i].value))) + "&";
			else
